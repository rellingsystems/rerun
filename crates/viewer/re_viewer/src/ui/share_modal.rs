...
pub struct ShareModal {
    ...
    ///
    /// This is shown on pressing the download button and reset when the download completes.
    show_copied_feedback: bool,
}
...
impl ShareModal {
    ...
    pub fn button_ui(
        ...
        // Change button text based on platform
    ...
    pub fn ui(
        ...
        // Change modal title based on platform
        ...
        self.modal.ui(
            ...
            |ui| {
                ...
                // Different button behavior for wasm vs native
                if cfg!(target_arch = "wasm32") {
                    // Download functionality for wasm
                    let download_button_label = if self.show_copied_feedback {
                        (
                            egui::Atom::grow(),
                        ...
                        (
                            egui::Atom::grow(),
                            icons::DOWNLOAD.as_image().tint(ui.tokens().icon_inverse),
                            "Download RRD",
                            egui::Atom::grow(),
                        )
                            .into_atoms()
                    ...
                    let download_button_response = ui
                        .scope(|ui| {
                            let tokens = ui.tokens();
                            let visuals = &mut ui.style_mut().visuals;
                            visuals.override_text_color = Some(tokens.text_inverse);
                    ...
                    // Original copy link functionality for native
                    let copy_link_label = if self.show_copied_feedback {
                        (
                            egui::Atom::grow(),
                            "Copied to clipboard!",
...
fn url_settings_ui(
    ...
    // Different UI text based on platform
