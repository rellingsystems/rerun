use egui::{AtomExt as _, IntoAtoms, NumExt as _, vec2};
use std::collections::{HashMap, HashSet};

use re_log_types::AbsoluteTimeRange;
use re_redap_browser::EXAMPLES_ORIGIN;
use re_ui::{
    UiExt as _, list_item::PropertyContent,
...
impl ShareModal {
    ...
    pub fn ui(
        ...
        self.modal.ui(
            ...
            |ui| {
                ui.set_max_height(
                    (ui.ctx().screen_rect().height() - 100.0)
                        .at_least(0.0)
                        .at_most(640.0),
                );
                ...
                if cfg!(target_arch = "wasm32") {
                    // --- RRD Download Button ---
                    ui.add_space(8.0);
                    let rrd_button_response =
                        ui.add_enabled_ui(startup_options.rrd_url.is_some(), |ui| {
                            let downloading =
                                download_feedback.get("rrd").copied().unwrap_or(false);
                    ...
                    rrd_button_response
                        .response
                        .on_disabled_hover_text("RRD file URL not provided.");
                    ...
                    // --- Video Button ---
                    ui.add_space(4.0);
                    let video_button_response =
                        ui.add_enabled_ui(startup_options.mp4_url.is_some(), |ui| {
                            let downloading =
                                download_feedback.get("video").copied().unwrap_or(false);
                            let label = if downloading {
                                "Downloading Video..."
                            } else {
                    ...
                    video_button_response
                        .response
                        .on_disabled_hover_text("Video file URL not provided.");

                    // --- CSV Button ---
                    ui.add_space(4.0);
                    let csv_button_response =
                        ui.add_enabled_ui(startup_options.csv_url.is_some(), |ui| {
                            let downloading =
                                download_feedback.get("csv").copied().unwrap_or(false);
                            let label = if downloading {
                    ...
                    csv_button_response
                        .response
                        .on_disabled_hover_text("Coordinates file URL not provided.");

                    // --- JSON Button ---
                    ui.add_space(4.0);
                    let json_button_response =
                        ui.add_enabled_ui(startup_options.json_url.is_some(), |ui| {
                            let downloading =
                                download_feedback.get("json").copied().unwrap_or(false);
                            let label = if downloading {
                                "Downloading Actions..."
                            } else {
                                "Download Annotations Actions"
                            };
                            if primary_button_style(ui, label).clicked() && !downloading {
                    ...
                    json_button_response
                        .response
                        .on_disabled_hover_text("Actions file URL not provided.");
                } else {
                    // --- NATIVE SHARE BUTTON ---
                    if let Some(url) = &mut self.url {
                        let url_string = {
                            ...
                            let url_string =
                                url.sharable_url(web_viewer_base_url).unwrap_or_default();
                        ...
                        let label: egui::WidgetText = if *show_copied_feedback {
                            "Copied!".into()
                        } else {
                            "Copy link".into()
                        };
...
fn time_range_ui(
    ...
    ui.list_item_flat_noninteractive(PropertyContent::new(trim_text).value_fn(|ui, _| {
        ui.selectable_toggle(|ui| {
            ...
            let (entire_tooltip, trim_tooltip) = if cfg!(target_arch = "wasm32") {
                (
                    "Export will include the entire recording.",
                    "Export will include only the selected time range.",
                )
            } else {
                (
                    "Link will share the entire recording.",
                    "Link trims the recording to the selected time range.",
                )
            };
    ...
    *url_time_range = if entire_range {
        None
    } else {
...
fn time_cursor_ui(
    ...
    let time_ctrl = rec_cfg.time_ctrl.read();
    let current_time_cursor = time_ctrl
        .time_cell()
        ...
        ui.selectable_toggle(|ui| {
            ...
            ui.add_enabled_ui(current_time_cursor.is_some(), |ui| {
                ...
                if let Some((_, time_cell)) = current_time_cursor {
                    label.push_right(
                        egui::RichText::new(time_cell.format(timestamp_format))
                            .weak()
                            .small()
                            .atom_shrink(true),
                    );
